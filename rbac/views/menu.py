...
def menu_list(request):
    ...
    # 二级,三级菜单
    second_menu_exists = models.Permission.objects.filter(pid_id=second_menu_id)
    ...
    if second_menu_id:
        ...
        permission_menu = []
    return render(request, 'rbac/menu_list.html', {'menus': menus, 'mid': mid, 'second_menu': second_menu,
...
def multi_permission(request):
    ...
    delete_row_list = [row_dict for name, row_dict in all_url.items() if name in delete_name_list]
