...
__all__ = ['Account', 'Course', 'Degree', 'CourseDetail', 'CourseOutline', 'CourseChapter', 'CourseLesson', 'Teacher',
           'Coupon', 'CouponDetail', 'Category', 'CommonQuestion', 'Comment', 'Order', 'OrderDetail', 'PricePolicy',
           'TradeRecord']
...
class Course(models.Model):
    ...
    degree_course = models.ForeignKey(to='Degree', on_delete=models.CASCADE, help_text='如果是学位课程,必须关联学位表', blank=True,
                                      null=True, verbose_name='学位课程')
...
class CourseChapter(models.Model):
    ...
    class Meta:
        ...
        # 章节和课程联合唯一，一个课程不能有多个相同章节


class Degree(models.Model):
    '''学位课程表'''
    title = models.CharField(max_length=32, verbose_name='学位课程表')
...
class CourseLesson(models.Model):
    ...
    course_lesson_type = models.SmallIntegerField(choices=course_lesson_type_choices, verbose_name='课时类型', default=0)
    free_trail = models.BooleanField(verbose_name='是否可试看', default=False)
    lesson_link = models.CharField(max_length=255, verbose_name='课程链接', help_text='如果是文档,需要给链接', blank=True, null=True)
...
class Teacher(models.Model):
    '''讲师表'''
    username = models.CharField(max_length=32, verbose_name='讲座姓名')
...
class Coupon(models.Model):
    ...
    brief = models.TextField(max_length=128, verbose_name='优惠券名称', blank=True, null=True)
    coupon_type_choices = ((0, '通用券'), (1, '折扣券'), (2, '满减券'))
    ...
    off_percent = models.PositiveIntegerField(verbose_name='折扣券百分比', blank=True, null=True, default=100,
                                              help_text='仅在折扣券使用')
    minimum_consume = models.FloatField(verbose_name='最低消费', blank=True, null=True, default=0, help_text='仅在满减券使用')
    grant_begin_time = models.DateTimeField(verbose_name='活动领取开始时间')
    grant_end_time = models.DateTimeField('活动领取结束时间')
...
class PaymentRecord(models.Model):
    ...
    confirm_user = models.ForeignKey(verbose_name='审批人', to='Account', related_name='confirms',
                                     on_delete=models.CASCADE, null=True, blank=True, limit_choices_to={'level': 0})
