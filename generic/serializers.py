...
from generic.models import Category, Course, CourseDetail, CourseChapter, Comment, CommonQuestion, StudyQuestion, \
    HomeworkDetail, Article
...
class CourseSerializer(serializers.ModelSerializer):
    ...
    '''
    course_img本来可以直接获取,但是django配置了media文件夹+model表设置了ImageField字段在获取图片时会自动添加[media/]路径在正式部署时,注释掉这个部分
    '''
    ...
    def get_free_course(self, obj):
        res = obj.course_chapters.first().course_lesson.all()
        return [{"title": lesson.title} for lesson in res if lesson.free_trail]

    class Meta:
        model = Course
        fields = ['id', 'title', 'course_img', 'status', 'study_number', 'price', 'price_policy', 'lesson', 'teacher',
                  'free_course']
