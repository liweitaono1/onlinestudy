...
class SettlementView(APIView):
    ...
    def delete_func(self, user_id, course, res):
        key = SETTLEMENT_KEY % (user_id, course, res)
        global_key = GLOBAL_COUPON_KEY % user_id
        if key:
            if not CONN.exists(key):
                res.code = 1090
...
class PaymentView(APIView):
    ...
    def get(self, request):
        ...
        for item in user_orders:
            ...
            product_dict[item.order_id][item.order_id] = product
    ...
    def post(self, request):
        ...
        if balance and int(balance) > request.user.balance:
            res.code = 1100
            res.error = '折扣贝里失败,贝里余额不足'
            return Response(res.dict)

        if price and isinstance(price, str):
            price = eval(price)
        # 购买有两种情况,一种直接购买,一种是添加购物车之后购买,已处理号此情况
        ...
        for item in user_settlements:
            ...
            # 检验课程合法性
            course_obj = models.Course.objects.filter(id=course_id).first()
            if not course_obj and course_obj.state == 2:
                # 状态2表示课程下架
                res.code = 1101
            ...
            # 有这四种情况
            # 当前用户本来没有领取专项优惠券
            # 当前用户有专项优惠券但redis里的数据不匹配
            # 当前用户有专项优惠券当并没有勾选使用优惠券
            # 正常使用专项优惠券
            if course_coupon_id:
                user_coupon_dict = models.Coupon.objects.filter(id=course_coupon_id, object_id=course_id,
        ...
        if user_global_coupon:
            ...
            if global_coupon_id:
                global_coupon_dict = models.Coupon.objects.filter(id=global_coupon_id,
                                                                  start_time__lte=now(),
                                                                  end_time__gte=now(),
        ...
        if global_coupon_key:
            CONN.delete(global_coupon_key)
            # 此处不用判断是否有优惠券,redis删除,如果没有值不会报错
        ...
        # 使用余额交易
        models.TradeRecord.objects.create(account_id=user.id, amount=balance, balance=user_obj.first().balance, )
        # 用其他方式交易
        ...
        for item in product:
            ...
            # 加入到缴费申请表
            models.PaymentRecord.objects.create(account=user_obj.first(), paid_fee=total_price, course=pay_course_obj)
            # 购买的用户信息链接到学生表里
    ...
    def delete_func(self, user, orders, res):
        # 只删除交易记录,不删除用户余额使用情况,可以正常查询用户的账号余额走向
...
class CouponDistributionView(APIView):
    ...
    def post(self, request):
        ...
        obj2 = models.CouponDetail.objects.create(coupon=coupon_obj, number=str(time.time()), account=request.user,
                                                  get_time=now())
...
class UserCourseView(APIView):
    ...
    def get(self, request):
        # 获取用户的账单
        res = BaseResponse()
        order_res = PaymentView().get(request)
        # 当前用户账单数据
