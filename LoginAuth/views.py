...
from generic.models import Account, Order
...
def account_trend(request, year):
    ...
    count = []
    for month in months:
        data = Account.objects.filter(date__year=year, date_month=month).count()
        count.append(data)
...
def order_trend(request, year):
    ...
    orders = []
    for month in months:
        data = Order.objects.filter(date__year=year, date__month=month).count()
        orders.append(data)
